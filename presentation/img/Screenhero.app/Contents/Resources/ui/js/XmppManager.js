...
//        console.log("presence: " + from);
    ...
    this.onConnect = function(status, reason, connection)
    {
        if (connection != self.connection) {
            console.log("got a callback for the wrong connection, boo!")
            return;
        }
        ...
        if (status == Strophe.Status.CONNECTING || status == Strophe.Status.AUTHENTICATING) {
            ...
            if (Main.ra.get("loginState") == Main.loginStates.signingOut || Main.ra.get("loginState") == Main.loginStates.signedOut) return;
    ...
    this.doPing = function() {
        if (Main.GetLoginState() == Main.loginStates.signedIn)
        {
            ...
            var iq;
            try {
                iq = $iq({type: 'get', to: self.connection.jid.split("@")[1].split("/")[0], id: id}).c('ping', {xmlns: 'urn:xmpp:ping'});
            }
            catch (e) {
                console.log("XmppManager exception in doPing: " + e);
                iq = undefined;
            }
            if (iq !== undefined) {
                ...
                self.onConnect(Strophe.Status.DISCONNECTED, "ping-timeout");
            }
        }
    }
    ...
    this.Initialize = function () {
        ...
        self.connection = new Strophe.Connection("wss://ws-xmpp0.screenhero.com/websocket");
